...
import pygame, sys
from ryRealTimeAsr06_threading_eng import ryAsrStream, ryGet1secSpeechAndRecogItWithProb
from ryRealTimeAsr06_threading_eng import  ryQ, ryRecogQGet, ryStream
import os
import shutil
...
def withspeech():
    ...
    # Apple Movement
        ...
        history_label =font.render(ttl_history ,True , (255,255,0))
...
def upload_sound():
    root = tk.Tk()
    root.withdraw()
    file_path = filedialog.askdirectory(initialdir=r".")
    if os.path.isdir('./upload_sound/'+os.path.basename(file_path)):
